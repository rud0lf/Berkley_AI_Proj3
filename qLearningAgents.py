...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        """
        value = -1*float("inf")
        if len(self.getLegalActions(state))==0: #or len(self.qValues)==0:
            return 0.0
        else:
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        qValue = 0
        for feature in F.iterkeys():
            qValue+=self.weights[feature]*F[feature]
        return qValue
    ...
    def update(self, state, action, nextState, reward):
        ...
        for feature in F.iterkeys():
            self.weights[feature] = self.weights[feature] + self.alpha*F[feature]*difference
